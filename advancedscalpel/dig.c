...
static inline void add_carved_bytes(unsigned long long n) {
  g_total_carved_bytes += n;
}
...
static int fseeko_use_coverage_map(struct scalpelState *state, FILE *fp, off_t offset);
static off_t ftello_use_coverage_map(struct scalpelState *state, FILE *fp);
...
int digImageFile(struct scalpelState* state) {
  ...
  while ((bytesread = 
	  fread_use_coverage_map(state, readbuffer,
          ...
          count_io_read((unsigned long long)bytesread);
...
int carveImageFile(struct scalpelState* state) {
  ...
  add_carved_bytes((unsigned long long)byteswritten);
      }
  ...
  int shortread;
   ...
   if (state->useCoverageBlockmap) {
	 ...
	 fprintf(stdout, "fread using coverage map to skip %llu bytes.\n", bytestoskip);
       ...
       fseeko(stream, (off_t)bytestoskip, SEEK_CUR);
